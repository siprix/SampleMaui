#pragma warning disable IDE1006, IDE0060, IDE0063, IDE0017, IDE0028
...
public class AccountModel : INotifyPropertyChanged
{
    ...
    int refreshRegistration(uint newExpireTimeSec)
    {
        string cmd = (newExpireTimeSec != 0) ? "Register" : "Unregister";
        int err = (newExpireTimeSec != 0) ? parent_.Core.Account_Register(ID, newExpireTimeSec)
        ...
        //Update UI
        uint prevExpireTime = accData_.ExpireTime;
        accData_.ExpireTime = newExpireTimeSec;
        setRegState(RegState.InProgress);

        //Save changes
        if (prevExpireTime != accData_.ExpireTime)
    ...
    {
        parent_.Logs?.Print($"OnAccount accId:{ID} reg state changed {state}/{response}");
    ...
    void setRegState(RegState state)
    {
        ...
        NotifyPropertyChanged(nameof(CanRegister));
        NotifyPropertyChanged(nameof(CanUnRegister));
    }
...
public class AccountsListModel(ObjModel parent_) : INotifyPropertyChanged
{
    ...
    public int Delete(AccountModel acc)
    {
        ...
        if (SelectedAccount == acc)
            SelectedAccount = (collection_.Count > 0) ? collection_[0] : null;
    ...
    public void StoreToJson()
    {
        ...
        Preferences.Default.Set("accounts", JsonSerializer.Serialize(jsonList));
    }
    public void LoadFromJson()
    {
        parent_.Logs?.Print("Loading accounts...");
        string jsonString = Preferences.Default.Get("accounts", "");
...
public class CallModel : INotifyPropertyChanged
{
                    ...
                    ObjModel parent)
    {
        ...
        AcceptCommand   = new Command(() => Accept(true));
        RejectCommand   = new Command(() => Reject());
        SwitchToCommand = new Command(() => SwitchTo());
        HoldCommand     = new Command(() => Hold());
        MuteMicCommand  = new Command(() => MuteMic(!micMuted_));
        MuteCamCommand  = new Command(() => MuteCam(!camMuted_));
        HangupCommand   = new Command(() => Bye());
    }
    ...
    public string RemoteExt         { get { return remoteExt_; } }
    public string DisplName         { get { return displName_; } }
    ...
    public void setDisplName(string name) { displName_ = name;  NotifyPropertyChanged(nameof(DisplName)); }
...
public class SubscriptionsListModel(ObjModel parent_)
{
    ...
    public void StoreToJson()
    {
        ...
        Preferences.Default.Set("subscriptions", JsonSerializer.Serialize(jsonList));
    }

    public void LoadFromJson()
    {
        parent_.Logs?.Print("Loading subscriptions...");
        string jsonString = Preferences.Default.Get("subscriptions", "");
...
public class MessageModel : INotifyPropertyChanged
{
    ...
    public bool SentSuccess { get; private set; } = false;
...
public class MessagesListModel(ObjModel parent_)
{
    ...
    public void StoreToJson()
    {
        ...
        Preferences.Default.Set("messages", JsonSerializer.Serialize(jsonList));
    }
    public void LoadFromJson()
    {
        string jsonString = Preferences.Default.Get("messages", "");
        if (jsonString.Length != 0)
        {
            collection_.Clear();
            using (JsonDocument document = JsonDocument.Parse(jsonString))
...
public class CdrModel : INotifyPropertyChanged
{
    ...
    public CdrModel(CallModel c)
    {
        ID = c.ID;
        AccUri = c.AccUri;
        RemoteExt = c.RemoteExt;
        DisplName = c.DisplName;
...
public class CdrsListModel(ObjModel parent_, int maxItems = 10)
{
    ...
    public void LoadFromJson()
    {
        ...
        string jsonString = Preferences.Default.Get("cdrs", "");
        if (jsonString.Length!=0)
        {
            collection_.Clear();
            using (JsonDocument document = JsonDocument.Parse(jsonString))
    ...
    public void StoreToJson()
    {
        ...
        Preferences.Default.Set("cdrs", JsonSerializer.Serialize(jsonList));
    }
...
public class ObjModel
{
    ...
    readonly ICoreService coreService_ = null!;
    private EventsHandler? eventHandler_;

    public ObjModel(ICoreService service)
    {
        coreService_ = service;
    ...
    public ICoreService Core { get { return coreService_; } }
    public string ErrorText(int err) { return coreService_.ErrorText(err); }

    public async void Initialize(IDispatcher dispatcher)
    {
        if (coreService_.IsInitialized())
            return;
        ...
        int err = await coreService_.Initialize(eventHandler_, iniData);

        if (err == ErrorCode.kNoErr) {
            Logs?.Print("Siprix module initialized successfully");
            Logs?.Print($"Version: {coreService_.Version()}");
            ...
            await WriteAssetAndGetFilePath("ringtone.mp3");
        }
        else {
    ...
    {
        int err = coreService_.UnInitialize();
        if (err == ErrorCode.kNoErr)
    ...
    {
        eventHandler_?.dispatcher_?.Dispatch(new Action(() => { accountsListModel_.StoreToJson(); }));
    }

    internal void PostSaveSubscriptionChanges()
    {
        eventHandler_?.dispatcher_?.Dispatch(new Action(() => { subscrListModel_.StoreToJson(); }));
    }

    internal void PostSaveMessagesChanges()
    {
        eventHandler_?.dispatcher_?.Dispatch(new Action(() => { messagesListModel_.StoreToJson(); }));
    }

    internal void PostSaveCdrChanges()
    {
        eventHandler_?.dispatcher_?.Dispatch(new Action(() => { cdrsListModel_.StoreToJson(); }));
    }

    internal void PostResolveContactName_(CallModel newCall)
    {
        eventHandler_?.dispatcher_?.Dispatch(new Action(() => {
            //string str = newCall.NameAndExt;
            //TODO 'ResolveContactName' and invoke //newCall.setDisplName
        }));
    }

    internal static async Task<string> WriteAssetAndGetFilePath(string assetName)
    {
        string targetFile = Path.Combine(FileSystem.Current.CacheDirectory, assetName);
        if(File.Exists(targetFile)) return targetFile;

        try
        {
            Stream fileStream = await FileSystem.Current.OpenAppPackageFileAsync(assetName);

            using Stream outFileStream = File.Create(targetFile);
            await fileStream.CopyToAsync(outFileStream);
            return targetFile;
        }
        catch (Exception)
        {
    ...
    //Events raised by SDK
    class EventsHandler(ObjModel parent_, IDispatcher dispatcher) : Siprix.IEventDelegate
    {
        readonly public IDispatcher dispatcher_ = dispatcher;

        public void OnTrialModeNotified()
        {
            dispatcher_?.Dispatch(new Action(() => {
